...
#table(bdata$behavc) |> plot
#(table(bdata$behavc) / sum(table(bdata$behavc)) ) |> plot

# define valid raw behaviors (I take out the undefined UWs or Ss)
behavs_c <- c("RUW", "RUW-NU", "RW", "RS", "SR", "GAL", "GP", "SR", "SRS", "NU", "Nu", "OR",
              "TSUW", "TSUW-OR", "TSS", "TSS-OR", "TSS->", #"TS", "TSY",
              "TMUW", "TNUW", "TMUW-OR",
              "TMS", "TMS-OR", "TFS", "TFUW", "TMUW-OR",
              "SA", "AS", "SAOR",
              "AUW", "UWA", "AW")
...
bdata$behavc_clust[bdata$behavc %in% c("RS", "SR", "GAL", "GP", "SR", "SRS", "NU", "Nu", "OR")] <- "R_S"
bdata$behavc_clust[bdata$behavc %in% c("RUW", "RW", "RUW-NU")] <- "R_UW"

bdata$behavc_clust[bdata$behavc %in% c("TSS", "TSS-OR", "TSS->")] <- "ST_S"
bdata$behavc_clust[bdata$behavc %in% c("TSUW", "TSUW-OR")] <- "ST_UW"

bdata$behavc_clust[bdata$behavc %in% c("TMS", "TMS-OR", "TFS")] <- "FT_S"
bdata$behavc_clust[bdata$behavc %in% c("TMUW", "TFUW", "TMUW-OR")] <- "FT_UW"

bdata$behavc_clust[bdata$behavc %in% c("SA", "AS", "SAOR")] <- "IPA_S"
bdata$behavc_clust[bdata$behavc %in% c("AUW", "UWA", "AW")] <- "IPA_UW"
...
bdata$uw[bdata$behavc %in% c("UW", "TUW")] <- 1
...
fol_shorts <- fol_agg$follow[fol_agg$cons_num < 3] # was 4
...
# number of follows
(N_fol <- length(unique(bdata$follow))) # 1097
...
(mean(fol_agg$t *5)) # mean follow duration = 53.9 min
...
B <- 3 # number of basis functions including the intercept
...
Bz <- 3 # number of basis functions including the intercept
...
sum(bdata$uw) / sum(is.na(bdata$behavc_clust)) # 95 % of NA are UW
sum(bdata$uw) / nrow(bdata) # 8.4 % of obs are UW
...
sdata <- list(
  ...
  # dec refers to decay, inc to increase (they live in logit scale);
  # There are three increase conditions: _c, _b, _m: attack to calf, both, and
  # mother.
  prior_dec_sds = c(1, 1.5, 1.5),
  prior_inc_sds = c(1, 1.5, 1.5),
...
# <behaviour_calves_analysis_prior_simulations.R>

# compile and sample
bm_code <- stan_model(file = "behaviour/behaviour_calves_model.stan", verbose = TRUE)
warmup <- 500
N_samples <- 1000
bm <- sampling(
  bm_code, seed = 5454, refresh = 1,
  chains = 8, cores = 8, iter = warmup + N_samples, warmup = warmup,
  # chains = 1, cores = 1, iter = 2, # used to test the stan code and data
  data = sdata,
  control = list(adapt_delta = 0.95, max_treedepth = 15),
  pars = c(
    "sd_spline_tmat", "sd_years",
    "decay",
    "increase_m",
    "increase_b", "increase_b3",
    "increase_c", "increase_c3",
    "kappa_c", "kappa_b", "kappa_m", "lambda_c",
...
summary(bms[, "n_eff"])
summary(bms[, "Rhat"])
saveRDS(bm, "models/behaviour/behaviour_calves_model_samples.rds")
saveRDS(sdata, "models/behaviour/behaviour_calves_model_data.rds")
# done

# it took 8309.79/3600 = 2.30 h // before it was 38.64 min. all perfect
...
(get_elapsed_time(bm) |> rowSums() |> max()) / 3600 # 2.308275 h running
  ...
  for(f in 1:N_fol) {
    z_samples[int_start[f], i] <- z0_samples[i, f] +
      ...
      bdata$a[int_start[f]] * (1 - z0_samples[i, f]) * plogis(
        ...
        kappa_c[i, sdata$year[int_start[f]]] * bdata$ac_only[int_start[f]] +                           
      
        kappa_b[i, sdata$year[int_start[f]]] * bdata$ab[int_start[f]] +
    
        kappa_m[i, sdata$year[int_start[f]]] * bdata$am_only[int_start[f]] +                           
    ...
    for(t in (int_start[f] + 1):int_end[f]) {
      ...
      z_samples[t, i] <- z_samples[t-1, i] +
        ...
        bdata$a[t] * (1 - z_samples[t-1, i]) * plogis(
          ...
          kappa_c[i, sdata$year[t]] * bdata$ac_only[t] +                           
            
          kappa_b[i, sdata$year[t]] * bdata$ab[t] +
            
          kappa_m[i, sdata$year[t]] * bdata$am_only[t] +                           
...
bdata$behavc_full[is.na(bdata$behavc_clust)] <- bdata$behavc_hat[is.na(bdata$behavc_clust)]

# check there are not missing behaviours
which(is.na(bdata$behavc_full)) # perfect

# View(bdata[bdata$follow %in% bdata$follow[is.na(bdata$behavc_full)],
#            c("follow", "t", "behavc_clust", "uw", "behavc_hat", "missing_obs")])
...
# , "behav", into = c("behav_raw", "position"), 
#                         sep = "_", remove = FALSE)
# predictions$behav_raw <- factor(predictions$behav_raw, levels = c(behavs_c_short, "z"),
#                                 labels = c(behavs_labels, "z"))
# predictions$position <- factor(predictions$position, levels = c("S", "UW"),
#                                labels = c("Surface", "Under water"))

# guardo tabla
saveRDS(predictions, "behaviour/files/behaviour_calves_predictions_attack scenarios table.rds")
predictions <- readRDS("behaviour/files/behaviour_calves_predictions_attack scenarios table.rds")
...
predictions$position <- as.character(predictions$position)
predictions$position[predictions$behav_raw == "z"] <- "both"
...
ggplot(predictions[filt_z, ], 
  ...
  geom_line() +
  geom_ribbon(color = NA, alpha = 0.15) +
  #facet_wrap(cols = vars(behav_raw, position), rows = vars(year)) +
...
# save
saveRDS(predictions_avg, "behaviour/files/behaviour_calves_predictions_attack scenarios table_avg.rds")
predictions_avg <- readRDS("behaviour/files/behaviour_calves_predictions_attack scenarios table_avg.rds")
...
ggsave("behaviour/figures/behaviour_calves_ts_free_y.png", height = 25, width = 20, unit = "cm")


# RAN ALL THE PREVIOUS, CONTINUE HERE -------------------------------------
...
pred_dist_samples[int_start, ] <- as.numeric(bdata$behavc_full[int_start])
...
pred_dist_wide <- readRDS("behaviour/files/behaviour_calves_model_pred dist wide.rds")
...
delta_wide <- readRDS("behaviour/files/behaviour_calves_predictions_total effects.rds")
...
ggplot(delta_wide, 
  ...
  xlab("Year")
ggsave("behaviour/figures/behaviour_calves_ts effects total.png",
...
# Hipothesys: some short and long term effects offset (they have sign)
...
ggplot(eff_wide[eff_wide$effect_type %in% selected, ], 
  ...
  scale_x_continuous(breaks = c(1995, 2005, 2008, 2012, 2016)) 
ggsave("behaviour/figures/behaviour_calves_ts effects short and long.png",
...
tmat_data$behav_raw_prev <- factor(tmat_data$behav_raw_prev, levels = c(behavs_c_short),
                                   labels = c(behavs_labels))
tmat_data$position_prev <- factor(tmat_data$position_prev, levels = c("S", "UW"))

tmat_data$behav_raw_post <- factor(tmat_data$behav_raw_post, levels = c(behavs_c_short),
                                   labels = c(behavs_labels))
...
ggsave("behaviour/figures/behaviour_calves_tmat few years.png", height = 20, width = 25,
       units = "cm")
...
# memory trials
# lilili <- readRDS("models/behaviour/behaviour_calves_model_samples_likelihood.rds")
...
likeplot <- separate(likeplot, "behav", into = c("behav_raw", "position"), 
                        sep = "_", remove = FALSE)
likeplot$behav_raw <- factor(likeplot$behav_raw, levels = c(behavs_c_short),
                                labels = c(behavs_labels))
