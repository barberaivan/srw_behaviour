...
data {
  ...
  vector<lower = 0>[Bz] prior_incm_sds;    // mother
  vector<lower = 0>[Bz] prior_incbc_sds;   // both and calf
  real<lower = 0> prior_lambdac_sd;
  
  // Lower limits for decay and increase_m
...
transformed parameters {
  ...
  vector[N_years] kappa;   // reference: attack to mother (already restricted so that increase_m lays in [L_inc, 1])
  vector[N_years] delta_c; // difference from kappa (-) when attacks only to calf
  vector[N_years] delta_b; // difference from kappa (+) when mother and calf are attacked
  // decay
  vector[N_years] decay;       // in [L_dec, 1] 
  
  // increase_m (p scale)
  vector[N_years] increase_m;  // in [L_inc, 1]
  
  // derived quantities to access easily:
  vector[N_years] increase_b;  // 1 attack to both
  vector[N_years] increase_b3; // 1 attack to mother, 3 to calf
  vector[N_years] increase_c;  // 1 attack to calf
  ...
  /* 
     ...
     Columns order [1:4]: decay, increase_m, delta_b, delta_c
     rows order [1:Bz]: quadratic, linear, intercept.
     (priors vary between rows and columns; 
     delta_b and delta_c are used to compute increase_b and increase_c)
  */
  zpar_spline_coef[, 1] = zpar_spline_coef_raw[, 1] .* prior_dec_sds;
  zpar_spline_coef[, 2] = zpar_spline_coef_raw[, 2] .* prior_incm_sds;
  zpar_spline_coef[, 3] = zpar_spline_coef_raw[, 3] .* prior_incbc_sds;
  zpar_spline_coef[, 4] = zpar_spline_coef_raw[, 4] .* prior_incbc_sds;
  ...
  increase_m = inv_logit(zpar_spline_raw[, 2]) * (1 - L_inc) + L_inc;
  kappa = logit(increase_m); // this way, kappa lives in the logit scale but
                             // meeting the constraint above.
  
  // deltas have to be positive
  delta_b = exp(zpar_spline_raw[, 3]);
  delta_c = exp(zpar_spline_raw[, 4]);
  
  // derived quantities to access easily:
  increase_b = inv_logit(kappa + delta_b);                 // 1 attack to both
  increase_b3 = inv_logit(kappa + delta_b + lambda_c * 2); // 1 attack to mother, 3 to calf
  increase_c = inv_logit(kappa - delta_c);                 // 1 attack to calf
  increase_c3 = inv_logit(kappa - delta_c + lambda_c * 2); // 3 attacks to calf
  ...
  for(f in 1:N_fol) {
    z[int_start[f]] = z_0[f] 
      ...
      + a[int_start[f]] * (1 - z_0[f]) * inv_logit(
        ...
        kappa[year[int_start[f]]]                           
        
        // if both are attacked, the intercept increases:
        + delta_b[year[int_start[f]]] * ab[int_start[f]] 
        
        // if only calf is attacked, the intercept decreases:
        - delta_c[year[int_start[f]]] * ac_only[int_start[f]]                           
