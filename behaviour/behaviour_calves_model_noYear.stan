data{
  ...
  // number of attacks to calf or mother not included because their effect 
  // copied the prior
  ...
  // Standard deviations of transition matrix parameters
  real<lower = 0> prior_sd_years_sd;

  // z parameters prior sd
  real<lower = 0> prior_zpar_sd;
  
  // lower limit for increase and decay (transformed) parameters
...
parameters{
  ...
  // z dynamics (without number of attacks)
  real<lower = L_dec, upper = 1> decay;
  real<lower = L_inc, upper = 1> increase_c; // increase when only calf is attacked
  real<lower = 0> delta_b_raw;  // difference (+) from kappa_c when both are attacked
  real<lower = 0> delta_m_raw;  // difference (-) from (kappa_c + delta_b = kappa_b) when only mother is attacked 
...
transformed parameters{
  ...
  // N_years transition matrices without reference row for alpha
  matrix[K-1, K] alpha_sub [N_years];
  // complete transition matrices
  matrix[K, K] alpha [N_years];
  matrix[K, K] beta; // does not vary among years
  ...
  vector<lower = 0, upper = 1>[N] z; // latent variable

  // z increase parameters at logit scale
  real kappa_c; // attack to calf
  real delta_b; // difference from kappa (+) when attacks only to calf
  real delta_m; // difference from (kappa + delta_b) (-) when only mother is attacked
  real kappa_b; // both are attacked
  real kappa_m; // only mother is attacked
  
  // derived quantities to access easily (in [L_inc, 1]):
  real increase_b;  // attack to both
  real increase_m;  // attack to mother
  
  //// Design matrix elements
  matrix[N_cons, K] y_mat_cons_z; // half desing matrix = y_mat_cons * z
  ...
  for(y in 1:N_years) {
    for(k in 1:K) {
      alpha_sub[y, , k] = alpha_intercept[, k] + alpha_raneff[y, k];  //matrix[N_years, K]
    }
  }
  ...
  for(k in 1:K) {
    // Fill off-diagonal elements
    beta[ind_matrix[, k], k] = beta_intercept[, k];       
    // Fill the diagonal with zeroes
    beta[k, k] = 0;
  }
  ...
  // Compute z parameters
    ...
    // z increase parameters at logit scale
  kappa_c = logit(increase_c);   // reference: attack to only calf, logit scale
                               // increase_c in [L_inc, 1] 
  delta_b = delta_b_raw * prior_zpar_sd; // difference from kappa (-) when attacks only to calf
  delta_m = delta_m_raw * prior_zpar_sd; // difference from kappa (+) when mother and calf are attacked
  kappa_b = kappa_c + delta_b; // kappa_b > kappa_c
  kappa_m = kappa_b - delta_m; // kappa_m < kappa_b,
                               // but kappa_m is not bounded from below as kappa_c.
  
  // derived quantities to access easily:
  increase_b = inv_logit(kappa_b);  // attack to both
  increase_m = inv_logit(kappa_m);  // attack to mother
  ...
  for(f in 1:N_fol) {
    z[int_start[f]] = z_0[f] 
      ...
      + a[int_start[f]] * (1 - z_0[f]) * inv_logit(
        kappa_c * ac_only[int_start[f]] +                          // attack calf (only)
        kappa_b * am_marg[int_start[f]] * ac_marg[int_start[f]] +  // attack both
        kappa_m * am_only[int_start[f]]                            // attack mother (only)
      )  
      // decay
      - (1 - a[int_start[f]]) * decay * z_0[f];
  }   
  /* 
    ...
    To compute effects of number of attacks, nam and nac are substracted 1 
    so the intercepts means logit(increase) when there is 1 attack, not 0, 
    because if nam and nac are 0, then increase has to be 0.
  */  
  ...
  for(f in 1:N_fol) {
    for(t in (int_start[f] + 1):int_end[f]) {
      ...
      + a[t] * (1 - z[t-1]) * inv_logit(
        kappa_c * ac_only[t] +               // attack calf (only)
        kappa_b * am_marg[t] * ac_marg[t] +  // attack both
        kappa_m * am_only[t]                 // attack mother (only)
      )  
      // decay
      - (1 - a[t]) * decay * z[t-1];
    }
...
model{
  ...
  // increase_c ~ unif(L_inc, 1); // implicit
  delta_m_raw ~ std_normal();
  delta_b_raw ~ std_normal();
  // decay ~ unif(L_dec, 1); // implicit
  ...
  {  // Local scope to define some variables that don't need to be saved
    ...
    for(j in 1:N_jumps) {
      ...
      for(t in start:end) {
        ...
        tmat_logit = alpha[year_local] + beta * z[t - 1]; // z at t-1 affects transition probabilities from t-1 to t.
  ...
  { // Local scope to define some variables that don't need to be saved
    ...
    for(j in 1:N_uw_ends) {
      ...
      for(t in start:end) {
        ...
        tmat_logit = alpha[year_local] + beta * z[t - 1]; // get previous z
        // Apply softmax over rows (r)
        for(r in 1:K) {
          tmat[r, ] = softmax(tmat_logit[r, ]')';
        }
