...
parameters {
  // coefficients
  vector[K] alpha_raw; // for p | ac_marg[t-1] = 0
  vector[K] beta_raw;  // for p | ac_marg[t-1] = 1
...
transformed parameters {
  ...
  //// coefficients
  vector[K] alpha = alpha_raw * prior_intercepts_sd_p; // for p | ac_marg[t-1] = 0
  vector[K] beta = beta_raw * prior_intercepts_sd_p;  // for p | ac_marg[t-1] = 1
...
model {
  ...
  
  {
    ...
    for(i in 1:N) {
      logit_p = X_behavc[i, ] * alpha * (1 - ac_marg_prev[i]) + // there was no attack before
                X_behavc[i, ] * beta * ac_marg_prev[i] +        // there was attack before
                e_fol_p[follow_id[i]] + 
                e_year_p[year_id[i]];
      ac_marg[i] ~ bernoulli_logit(logit_p);
    }
